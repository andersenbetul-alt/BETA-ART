      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div className="border border-[#1e1e2e]" data-reveal>
          <div className="grid grid-cols-12 border-b border-[#1e1e2e] bg-[#0e0e18]">
            <div className="col-span-4 md:col-span-3 p-4 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">Tier</div>
            <div className="col-span-5 md:col-span-6 p-4 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">Use</div>
            <div className="col-span-3 p-4 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660] text-right">Price</div>
          </div>
          {tiers.map(([tier, use, price, hi]) => (
            <div key={tier} className={`grid grid-cols-12 border-b border-[#1e1e2e] last:border-b-0 ${hi ? "bg-[#EF2B2D]/10" : ""}`}>
              <div className={`col-span-4 md:col-span-3 p-4 font-[Barlow_Condensed] font-black text-xl ${hi ? "text-[#EF2B2D]" : "text-[#F0EDE8]"}`}>{tier}</div>
              <div className="col-span-5 md:col-span-6 p-4 font-mono text-[11px] uppercase tracking-[0.15em] text-[#F0EDE8]/80 self-center">{use}</div>
              <div className="col-span-3 p-4 font-mono text-[11px] uppercase tracking-[0.2em] text-[#F0EDE8] text-right self-center">{price}</div>
            </div>
          ))}
        </div>
        <div className="mt-8">
          ...
          >
            {submitted ? (
              <div className="md:col-span-2 py-12 text-center">
                <div className="font-[Barlow_Condensed] font-black text-3xl text-[#EF2B2D]">INQUIRY RECEIVED.</div>
                <p className="mt-3 font-mono text-xs uppercase tracking-[0.2em] text-[#F0EDE8]">We'll reply within 48 hours to sync@hximusic.com.</p>
              </div>
            ) : (
              <>
                ...
                <div className="md:col-span-2">
                  <Label>Message</Label>
                  <textarea rows={4} className="w-full bg-[#080810] border border-[#1e1e2e] text-[#F0EDE8] font-mono text-xs px-4 py-3 focus:border-[#EF2B2D] outline-none" />
                </div>
                <div className="md:col-span-2">
                  <button type="submit" className="bg-[#EF2B2D] text-[#F0EDE8] font-mono text-xs uppercase tracking-[0.25em] px-6 py-3.5">Send Sync Inquiry →</button>
                </div>
              </>
            )}
          </form>
...
function Input({ label, type = "text", required, placeholder }: { label: string; type?: string; required?: boolean; placeholder?: string }) {
  return (
    <div>
      <Label>{label} {required && <span className="text-[#EF2B2D]">*</span>}</Label>
      <input type={type} required={required} placeholder={placeholder} maxLength={200}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 grid md:grid-cols-2 gap-12">
        <div data-reveal>
          <div className="font-[Barlow_Condensed] font-black text-[16vw] md:text-[10vw] leading-[0.85] text-[#EF2B2D]/90 tracking-tighter">
            THE<br />FRE-<br />QUEN-<br />CY.
          </div>
        </div>
        <div data-reveal>
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#EF2B2D] mb-4">◈ 006 — About</div>
          <div className="space-y-5 text-[#F0EDE8]/85 leading-relaxed">
            <p>HXI is a Norwegian producer, DJ, and sound designer whose glitch-laced, bitcrushed approach to phonk has made him one of the genre's defining underground voices. Active since 2021, his sound moves between hard phonk, drift trap, and electronic experimentation.</p>
            <p>His breakout single 'help urself' surpassed 43 million streams on Spotify — earned organically through the anime and gaming edit community. Zero paid promotion. His music has placed in the Fast & Furious franchise and on the official BODYCAM game soundtrack, where he also handled mix and mastering independently.</p>
            <p>A NoCopyrightSounds artist since 2024. Based in Oslo. Active globally.</p>
          </div>
          <div className="mt-8 border border-[#EF2B2D]/50 p-5">
            <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#EF2B2D] mb-2">Independent Artist · Market Position</div>
            <p className="text-[#F0EDE8]/80 text-sm">Major labels control 70% of streaming revenue. HXI operates with full master ownership, 100% royalty access, and direct sync licensing control — in a $37B+ streaming market growing at 15% annually.</p>
          </div>
          <div className="mt-6 flex flex-wrap gap-4">
            <a href="#" className="font-mono text-xs uppercase tracking-[0.25em] text-[#F0EDE8] hover:text-[#EF2B2D]">Download EPK ↓</a>
            <a href="mailto:press@hximusic.com" className="font-mono text-xs uppercase tracking-[0.25em] text-[#F0EDE8] hover:text-[#EF2B2D]">Press enquiries → press@hximusic.com</a>
          </div>
          <div className="mt-8 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">Collaborators: -Prey · TWISTED · Nateki · pvppet · ToufG · 310babii · Crazy Mano · Grioten</div>
        </div>
      </div>
    </section>
  );
          ...
          {items.map((i) => (
            <Card key={i.t}>
              <div className="aspect-square bg-[#080810] border border-[#1e1e2e] mb-5 flex items-center justify-center relative">
                <span className="font-[Barlow_Condensed] font-black text-6xl text-[#1e1e2e]">HXI</span>
                {i.badge && (
                  <span className={`absolute top-2 right-2 font-mono text-[9px] uppercase tracking-[0.2em] px-2 py-1 ${i.badge === "FREE" ? "bg-[#EF2B2D] text-[#F0EDE8]" : "bg-[#002868] text-[#F0EDE8]"}`}>
                    {i.badge}
                  </span>
                )}
              </div>
              <div className="font-[Barlow_Condensed] font-black text-xl text-[#F0EDE8] leading-tight">{i.t}</div>
              <div className="mt-1 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">{i.d}</div>
              <div className="mt-4 flex items-center justify-between">
                <span className="font-[Barlow_Condensed] font-black text-lg text-[#F0EDE8]">{i.p}</span>
                <button className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#EF2B2D]">{i.badge === "SOON" ? "Notify →" : "Get →"}</button>
              </div>
            </Card>
          ))}
        </div>
  ...
  return (
    <section className="py-24 md:py-32 bg-[#0e0e18] border-b border-[#1e1e2e]">
      <div className="max-w-4xl mx-auto px-4 md:px-8 text-center" data-reveal>
        <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#EF2B2D]">◈ 008 — Direct Line</div>
        <h2 className="mt-4 font-[Barlow_Condensed] font-black text-5xl md:text-7xl text-[#F0EDE8] leading-[0.9]">DROP FIRST.<br />WORLD LATER.</h2>
        <p className="mt-6 text-[#F0EDE8]/80 max-w-xl mx-auto">Releases, stem packs, unreleased tracks — direct to your inbox. Free 'help urself' stem pack on signup.</p>
        <form onSubmit={(e) => { e.preventDefault(); if (email.includes("@")) setOk(true); }} className="mt-10 flex flex-col sm:flex-row gap-0 max-w-xl mx-auto">
          ...
          <button type="submit" className="bg-[#EF2B2D] text-[#F0EDE8] font-mono text-xs uppercase tracking-[0.25em] px-8 py-4">Enter →</button>
        </form>
        {ok && <div className="mt-4 font-mono text-xs uppercase tracking-[0.2em] text-[#EF2B2D]">✓ You're in. Check your inbox.</div>}
        <p className="mt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">GDPR compliant · Unsubscribe anytime · No spam</p>
      </div>
    </section>
  );
}
...
function BookingSection() {
  ...
  return (
    <section id="booking" className="py-24 md:py-32 border-b border-[#1e1e2e]">
      <div className="max-w-7xl mx-auto px-4 md:px-8 grid md:grid-cols-2 gap-12">
        <div data-reveal>
          <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#EF2B2D]">◈ 009 — Booking</div>
          <h2 className="mt-3 font-[Barlow_Condensed] font-black text-5xl md:text-7xl text-[#F0EDE8] leading-[0.9]">BOOK HXI.</h2>
          <p className="mt-6 text-[#F0EDE8]/80">DJ sets and live performances rooted in hard phonk, drift, and Nordic industrial sound design. Clubs, festivals, brand events, virtual — worldwide.</p>
          <div className="mt-8 border border-[#1e1e2e] divide-y divide-[#1e1e2e]">
            {[["DJ Set", "60 · 90 · 120 min"], ["B2B Set", "With approved artists"], ["Virtual / Livestream", "Worldwide"], ["Studio Session", "Invitation only"]].map(([t, s]) => (
              <div key={t} className="flex justify-between p-4">
                <span className="font-[Barlow_Condensed] font-black text-xl text-[#F0EDE8]">{t}</span>
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660] self-center">{s}</span>
              </div>
            ))}
          </div>
        </div>
        <form onSubmit={(e) => { e.preventDefault(); setSent(true); }} className="border border-[#1e1e2e] bg-[#0e0e18] p-6 md:p-8 grid grid-cols-1 gap-4" data-reveal>
          {sent ? (
            <div className="py-16 text-center">
              <div className="font-[Barlow_Condensed] font-black text-3xl text-[#EF2B2D]">BOOKING RECEIVED.</div>
              <p className="mt-3 font-mono text-xs uppercase tracking-[0.2em] text-[#F0EDE8]">Reply within 48h to booking@hximusic.com.</p>
            </div>
          ) : (
            <>
              <Input label="Name" required />
              ...
              <div>
                <Label>Message *</Label>
                <textarea rows={4} required className="w-full bg-[#080810] border border-[#1e1e2e] text-[#F0EDE8] font-mono text-xs px-4 py-3 focus:border-[#EF2B2D] outline-none" />
              </div>
              <button type="submit" className="bg-[#EF2B2D] text-[#F0EDE8] font-mono text-xs uppercase tracking-[0.25em] px-6 py-4">Send Booking Inquiry →</button>
            </>
          )}
        </form>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        <div className="grid md:grid-cols-4 gap-10">
          <div className="md:col-span-1">
            <div className="font-[Barlow_Condensed] font-black text-6xl text-[#F0EDE8] leading-none">HXI</div>
            <div className="mt-4 font-mono text-[10px] uppercase tracking-[0.25em] text-[#666660]">Norway · Scar Scheme Records</div>
            <div className="mt-2 font-mono text-[10px] uppercase tracking-[0.25em] text-[#666660]">Registered: TONO · GRAMO · Norway</div>
          </div>
          <FooterCol title="Music" links={["Spotify", "Apple Music", "SoundCloud", "YouTube", "NCS"]} />
          <FooterCol title="Social" links={["Instagram @prod.hxi", "TikTok @hximusic", "X/Twitter @HXIMusic"]} />
          <FooterCol title="Contact" links={["booking@hximusic.com", "sync@hximusic.com", "press@hximusic.com"]} />
        </div>
        <div className="mt-16 pt-8 border-t border-[#1e1e2e] flex flex-col md:flex-row justify-between gap-4 font-mono text-[10px] uppercase tracking-[0.2em] text-[#666660]">
          ...
          <div className="flex gap-6 flex-wrap">
            {["Privacy Policy", "Terms", "Cookie Policy", "DMCA"].map((l) => (
              <a key={l} href="#" className="hover:text-[#F0EDE8]">{l}</a>
            ))}
          </div>
        </div>
      </div>
...
function FooterCol({ title, links }: { title: string; links: string[] }) {
  return (
    <div>
      <div className="font-mono text-[10px] uppercase tracking-[0.3em] text-[#EF2B2D] mb-4">{title}</div>
      <ul className="space-y-2.5">
        {links.map((l) => (
          <li key={l}><a href="#" className="font-mono text-xs text-[#F0EDE8]/80 hover:text-[#EF2B2D] transition">{l}</a></li>
        ))}
      </ul>
    </div>
  );
  ...
  return (
    <div className="fixed bottom-0 inset-x-0 z-50 bg-[#0e0e18] border-t border-[#EF2B2D] p-4 md:p-5">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center gap-4 justify-between">
        <p className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#F0EDE8]/80">◈ This site uses cookies for analytics. By continuing you accept the cookie policy.</p>
        <div className="flex gap-2">
          <button onClick={() => { localStorage.setItem("hxi-cookie", "declined"); setShow(false); }} className="border border-[#1e1e2e] text-[#F0EDE8] font-mono text-[10px] uppercase tracking-[0.2em] px-4 py-2">Decline</button>
          <button onClick={() => { localStorage.setItem("hxi-cookie", "accepted"); setShow(false); }} className="bg-[#EF2B2D] text-[#F0EDE8] font-mono text-[10px] uppercase tracking-[0.2em] px-4 py-2">Accept</button>
        </div>
      </div>
    </div>
  );
