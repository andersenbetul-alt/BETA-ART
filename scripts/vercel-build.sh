#!/bin/sh
# Vercel dağıtım tarifi — vercel.json'daki buildCommand buraya taşındı çünkü
# Vercel'in JSON şeması buildCommand'i 256 karakterle sınırlıyor (bu satır
# tek parça hâlindeyken 691 karakterdi, dağıtımı "schema validation failed"
# ile düşürüyordu). Davranış değişmedi — yalnızca konum.
set -e

git clone --depth 1 --filter=blob:none --sparse --branch main \
  https://github.com/andersenbetul-alt/BETA-ART-PRIVAT.git _src
