...
(function () {
  ...
  // Where an address is posted. Empty until a list exists, and while it is
  // empty the form says so rather than promising a letter nobody can send.
  ...
  if (subscribeForm) {
    subscribeForm.addEventListener("submit", function (e) {
      ...
      if (!SUBSCRIBE_ENDPOINT) {
        // No list exists yet. "Check your inbox for the confirmation letter"
        // sends the reader to wait for something nobody can send, and the
        // address is not stored anywhere either.
        if (subscribeStatus) {
          subscribeStatus.textContent = "There is no subscription list yet — nothing was stored. " +
            "Write to hallo@beta-art.com and you go on it the day it opens.";
        }
        return;
      }

      if (subscribeStatus) subscribeStatus.textContent = "Signing you up …";
      fetch(SUBSCRIBE_ENDPOINT, {
        ...
        if (subscribeStatus) {
          subscribeStatus.textContent = "That did not go through — nothing is stored. " +
            "Write to hallo@beta-art.com instead.";
        }
      });
    });
  }
